...
from utilis import save_to_json
...
def weather_ui():
    ...
    with st.form("weather_form"):
        ...
        with col2:
            unit = st.selectbox("Select Units", ["metric", "imperial"], 
                               help="metric = Celsius\n\nimperial = Fahrenheit")
    ...
    # Display weather data from session state
    if st.session_state.weather_data:
        st.divider()
        
        # Display weather data in expandable view
        with st.expander("View All Details"):
            st.json(st.session_state.weather_data)
        
        # Save button
        if st.button("Save Data to JSON File", use_container_width=True, type="primary"):
            try:
